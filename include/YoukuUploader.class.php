...
class YoukuUploader
{
	...
	public function upload($upload_process = true, $params = array(),$uploadInfo = array(), $video_id) { 
		...
		if ($finish) {
			...
			echo "Uploading success!<hr>\n";
			if (isset($commitResult->video_id)) {
				$db = new SQLite3('DatabaseVocabulary.db');
				$db->exec('UPDATE video SET youku_id = "' . $commitResult->video_id . '" WHERE video_id = ' . $video_id);
				$db->close();
				echo "youku_id: ". $commitResult->video_id . "<br>\n";
				echo "video_id = $video_id <br>\n";
				$youkuPlayerUrl = "https://openapi.youku.com/v2/videos/show_basic.json?client_id=f9e962f0927cea7d&video_id=" . $commitResult->video_id;
				echo "<button type='button' style='font-size: 12pt' onclick='getThumbnail(\"$youkuPlayerUrl\");'>Get Thumbnail</button><hr>\n";
				echo "<div></div>\n";
				echo "<script src=\"http://ajax.googleapis.com/ajax/libs/jquery/1.11.2/jquery.min.js\"></script>\n";
				echo "<script type='text/javascript'>\n";
				echo "\tfunction getThumbnail(url) {\n";
				//echo "\t\talert(url);\n";
				echo "\t\t$.getJSON(url, function(result){\n";
				echo "\t\t\t$.each(result, function(i, field){\n";
                echo "\t\t\t\tif (i == 'thumbnail') $('div').append(i + ' = ' + field + '<br>');\n";
				echo "\t\t\t});\n";
				echo "\t\t});\n";
				echo "\t}\n";
				echo "</script>\n";
